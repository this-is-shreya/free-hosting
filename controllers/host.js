...
module.exports.home = async (req, res) => {
  try {
    ...
    res.render("host", { userData: userData[0] });
  } catch (error) {
              ...
              image.imageName.apply(async (imageName) => {
                if (state == "update") {
                  await userSchema.findOneAndUpdate(
                    {
                      sites: { $elemMatch: { title: title } },
                    },
                    {
                      $set: {
                        "sites.$.imageName": imageName,
                        "sites.$.hostUrl": dnsName,
                      },
                    }
                  );
                } else {
...
module.exports.static = async (req, res) => {
  try {
    axios({
      method: "get",
      url: `https://api.github.com/user/repos`,
...
module.exports.hostStatic = async (req, res) => {
  try {
    await fse
